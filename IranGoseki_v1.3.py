...
def main():
    syuList = csv.reader(open("In.csv", encoding="utf_8-sig"))
    skillDict = dict(csv.reader(open("SkillList.csv", encoding="utf_8-sig")))
    ...
    f = open('Out.csv', 'w', encoding="utf_8-sig", newline='')
    writer = csv.writer(f)
    for row in syuList:
        writer.writerow(row)
    f.close()
